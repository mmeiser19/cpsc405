...
int cmain()
{
  ...
  mmuinit1();
  gpuinit();
  pinit();    // proc init - in proc.c
  tvinit();   // trap vector init - in trap.c
  cprintf("it is ok after tvinit\n");
  binit();    // buffer cache init - buffers for file systems - in bio.c
cprintf("it is ok after binit\n");
  fileinit(); // file system init - in file.c
cprintf("it is ok after fileinit\n");
  iinit();    // inode init - in fs.c
cprintf("it is ok after iinit\n");
  ideinit();  // interface between buff and disk
cprintf("it is ok after ideinit\n");
  kinit2(P2V((8*1024*1024)+PHYSTART), P2V(pm_size)); // kernel page table init - 2 phases
